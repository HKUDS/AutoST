...
class BioDataset1(InMemoryDataset):
        ...
        def processed_file_names(self):
            # return 'dataset_new_2.pt' #'geometric_data_processed.pt'
...
class BioDataset(InMemoryDataset):
    ...
    def processed_file_names(self):
        return 'dataset_new_2.pt' #'geometric_data_processed.pt'
        # return 'dataset_new_60.pt' #'geometric_data_processed.pt'
...
class BioDataset_graphcl(BioDataset):
    ...
    def generator_generate(self, data, n_generator=1):
        with torch.no_grad():
            prob, edge_attr_pred,type_predict = self.generators[n_generator-1].generate(data)
            # prob, edge_attr_pred = self.generators[n_generator-1].generate(data)
            prob = prob.numpy()
        ...
        edge_attr = edge_attr_pred[idx_edge[0].long(), idx_edge[1].long()]
        edge_type = type_predict[idx_edge[0].long(), idx_edge[1].long()]
